...
export default function ServicePageEditor() {
  ...
  // Per-trust-item custom icon upload → trustSection.items[i].iconImage.
  async function handleTrustIconUpload(i, file) {
    const key = `trust-item-${i}-icon`;
    const err = validateImageFile(file);
    if (err) { setUploadError(key, err); return; }
    clearUploadError(key);
    setUploadProgress((p) => ({ ...p, [key]: 0 }));
    try {
      ...
      setTrustSection((p) => ({ ...p, items: (p.items ?? []).map((it, idx) => (idx === i ? { ...it, iconImage: url } : it)) }));
              ...
              {(trustSection.items ?? []).map((item, i) => (
                <div key={i} className="rounded-xl border border-slate-200 bg-slate-50/60 p-3">
                  ...
                  <div className="grid gap-2">
                    ...
                    <div>
                      <label className={labelClass}>Custom Icon (optional — overrides the icon above)</label>
                      <div className="flex items-center gap-2">
                        <input
                          value={item.iconImage ?? ""}
                          onChange={(e) => setTrustSection((p) => ({ ...p, items: (p.items ?? []).map((it, idx) => (idx === i ? { ...it, iconImage: e.target.value } : it)) }))}
                          className={inputClass}
                          placeholder="Icon URL or upload"
                          maxLength={FIELD_LIMITS.link}
                        />
                        <label className="shrink-0 inline-flex items-center gap-1 rounded-lg border border-[#0088FF]/30 bg-[#EEF6FF] px-3 py-2 text-xs font-semibold text-[#0088FF] hover:bg-[#dcecff] cursor-pointer">
                          {uploadProgress[`trust-item-${i}-icon`] !== undefined ? (
                            <><Loader2 className="h-3.5 w-3.5 animate-spin" />{uploadProgress[`trust-item-${i}-icon`]}%</>
                          ) : (
                            <><Upload className="h-3.5 w-3.5" />Upload</>
                          )}
                          <input
                            ...
                            accept="image/*"
                            className="hidden"
                            disabled={uploadProgress[`trust-item-${i}-icon`] !== undefined}
                            onChange={(e) => {
                              const file = e.target.files?.[0];
                              if (file) handleTrustIconUpload(i, file);
                              e.target.value = "";
                            }}
                          />
                        </label>
                      ...
                      {uploadErrors[`trust-item-${i}-icon`] ? (
                        <p className="mt-1 text-xs font-medium text-red-600" role="alert">{uploadErrors[`trust-item-${i}-icon`]}</p>
                      ) : null}
                    </div>
                  </div>
                </div>
