...
import { ArrowLeft, Trash2, Mail, Phone, Building2, Layers, MessageSquare, FileText, ExternalLink } from "lucide-react";
...
/* ── Detail view ─────────────────────────────────────────────────────── */
function ProposalDetail({ proposal, onBack, onStatusChange, onDelete }) {
  ...
  const solutions = Array.isArray(proposal.solutions) ? proposal.solutions : [];
  ...
  async function handleStatusChange(newStatus) {
    ...
    setError("");
    try {
      const data = await api.updateProposalStatus(id, newStatus);
      const next = data.proposal?.status ?? newStatus;
      setStatus(next);
    ...
    <div>
      ...
      <div className="rounded-[20px] border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 px-6 py-5">
          <div>
            <h2 className="text-lg font-semibold text-[#050A13]">{proposal.fullName}</h2>
            {proposal.company ? (
              <p className="mt-0.5 text-sm text-slate-500">{proposal.company}</p>
            ) : null}
          </div>
        ...
        <div className="grid gap-5 px-6 py-5 sm:grid-cols-2">
          <DetailField icon={Mail}      label="Email"   value={proposal.email} href={`mailto:${proposal.email}`} />
          <DetailField icon={Phone}     label="Phone"   value={proposal.phone} href={`tel:${proposal.phone}`} />
          <DetailField icon={Building2} label="Company" value={proposal.company} />
          {solutions.length > 0 && (
            <div className="sm:col-span-2">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400">Solutions</p>
            ...
            <div className="sm:col-span-2">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-[#050A13]">{proposal.message}</p>
                </div>
              </div>
            </div>
          )}
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            {proposals.length} proposal{proposals.length !== 1 ? "s" : ""} · click a row to view details
          </p>
        </div>
      </div>
