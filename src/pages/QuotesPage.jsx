...
import { ArrowLeft, Trash2, Mail, Phone, Briefcase, Building2, Hash, MessageSquare, FileText, ExternalLink } from "lucide-react";
...
/* ── Detail view ─────────────────────────────────────────────────────── */
function QuoteDetail({ quote, onBack, onStatusChange, onDelete }) {
  ...
  async function handleStatusChange(newStatus) {
    ...
    setError("");
    try {
      const data = await api.updateQuoteStatus(quote._id, newStatus);
      setStatus(data.quote.status);
      onStatusChange(quote._id, data.quote.status);
    } catch (err) {
    ...
    <div>
      ...
      <div className="rounded-[20px] border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 px-6 py-5">
          <div>
            <h2 className="text-lg font-semibold text-[#050A13]">{quote.fullName}</h2>
            <p className="mt-0.5 text-sm text-slate-500">{quote.service}</p>
          </div>
        ...
        <div className="grid gap-5 px-6 py-5 sm:grid-cols-2">
          <DetailField icon={Mail}      label="Email"       value={quote.email}   href={`mailto:${quote.email}`} />
          <DetailField icon={Phone}     label="Phone"       value={quote.phone}   href={`tel:${quote.phone}`} />
          <DetailField icon={Briefcase} label="Service"     value={quote.service} />
          <DetailField icon={Building2} label="Tower / Location" value={quote.tower} />
          <DetailField icon={Hash}      label="Slots Needed" value={quote.slots} />
          {quote.message && (
            <div className="sm:col-span-2">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-[#050A13]">{quote.message}</p>
                </div>
              </div>
            </div>
          )}
                  ...
                  <div className="mt-1.5 flex flex-wrap gap-2">
                    {attachments.map((file, index) => (
                      <a
                        ...
                        className="inline-flex max-w-[240px] items-center gap-1.5 rounded-lg border border-[#0088FF]/30 bg-[#EEF6FF] px-3 py-1.5 text-xs font-semibold text-[#0088FF] hover:bg-[#0088FF] hover:text-white"
                      >
...
export default function QuotesPage() {
  ...
  if (loading) return <p className="text-slate-500">Loading quotes…</p>;
    ...
    <div>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            {quotes.length} quote{quotes.length !== 1 ? "s" : ""} · click a row to view details
          </p>
        </div>
      </div>
      ...
      {quotes.length === 0 ? (
        <div className="mt-8 rounded-[20px] border border-dashed border-slate-200 bg-white px-6 py-12 text-center text-sm text-slate-500">
          No quotes yet. Submissions appear here when visitors request a quote from the website.
        </div>
      ) : (
                ...
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="font-medium text-[#050A13]">{quote.fullName}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-slate-600">
                    {Array.isArray(quote.attachments) && quote.attachments.length > 0
                      ? `${quote.attachments.length} file${quote.attachments.length !== 1 ? "s" : ""}`
